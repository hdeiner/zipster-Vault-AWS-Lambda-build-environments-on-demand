resource "aws_instance" "awsqa_lambda_mysql" {
  ...
  security_groups = [aws_security_group.awsqa_lambda_mysql.name]
  tags = {
    Name = "${var.environment} MySQL Server ${format("%03d", count.index)}"
  }
  provisioner "local-exec" {
    command = "aws ec2 wait instance-status-ok --region ${regex("[a-z]+[^a-z][a-z]+[^a-z][0-9]+",self.availability_zone)} --instance-ids ${aws_instance.awsqa_lambda_mysql[count.index].id}"
  }
  provisioner "file" {
    connection {
      type = "ssh"
